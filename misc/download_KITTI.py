...
def download():
    subprocess.run("mkdir ./data/KITTI", shell=True)
    ...
    for item in download_list:
        if "calib" in item:
            ...
            nozipname = item
            shortname = item + "_sync.zip"
            longname = item + "/" + shortname
        ...
        subprocess.run(
            "unzip -o ./temp/" + shortname + " -d ./temp/" + nozipname, shell=True
        )

        date = next(
                d
                for d in (next(os.walk(os.path.join("./temp", nozipname)))[1])
                if not d[0] == "."
            )
        ...
        if "calib" in item:
            ...
            rsync_cmd = (
                "rsync -avhP ./temp/" + nozipname + "/" + date + "/" + +" ./data/KITTI"
            )
